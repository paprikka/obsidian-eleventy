...
filenames.forEach((filename) => {
  // Append 'vault/' to the filename
  const filePath = path.resolve(__dirname, "vault", filename);
  const fileContent = fs.readFileSync(filePath, "utf8");

  const parsed = grayMatter(fileContent, { lineWidth: 10000 }); // Disable automatic folding of URLs
  const data = parsed.data;

  // Check if 'publish' field is already present
  if (!data.hasOwnProperty("publish")) {
    data.publish = true;

    // Combine updated frontmatter with the original content
    const newContent = grayMatter.stringify(parsed.content, data, {
      lineWidth: 10000,
    });
